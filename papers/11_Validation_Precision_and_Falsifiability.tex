...
\begin{enumerate}
    ...
    \item CDFL model validates and reports zero blocking diagnostics;
    \item CDFL lint, format, and AST tooling agree with the parser surface;
...
python -m pytest -q tests/test_cli_runtime.py \
  ...
  tests/test_cdfl_vscode_extension.py
\end{verbatim}
with 38 passing focused tests. These tests cover CLI runner behavior, CDFL
parsing and execution, CDFL diagnostics/formatting/AST/sample helpers, Part II
diagnostics export, origins-of-life guardrails, provider-key redaction,
release-surface metadata, and the VS Code extension package metadata. A
separate Extension Development Host smoke exercised the \texttt{.cdfl} language
mode, commands, diagnostics, completions, symbols, formatting, validation, and
runtime execution.
