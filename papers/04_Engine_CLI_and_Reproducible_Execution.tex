...
it supports reproducible simulations, paper diagnostics, CDFL language tooling,
validation, export, reports, optional provider interpretation above saved
outputs, and automation before any visual layer is treated as authoritative.
\end{abstract}
...
python cdfd.py llm explain runs/<run>/result.json --dry-run
...
\section{Optional Provider Boundary}
The current CLI includes \texttt{cdfd.py llm} commands for research
interpretation of saved deterministic outputs. This layer is optional and sits
above the engine. \texttt{cdfd.py llm providers} lists the supported provider
shapes; \texttt{cdfd.py llm status} checks whether a provider key is configured
without making a provider call; \texttt{cdfd.py llm explain ... --dry-run}
builds a prompt audit without contacting a provider. A real provider call is
made only when \texttt{llm explain} is invoked without \texttt{--dry-run} and a
provider, model, base URL, and key state support the call.

Raw provider keys are never printed in result envelopes or reports. The
compatibility command \texttt{cdfd.py auth} is now an alias for provider-key
status, not a local runtime allowlist. The deterministic engine remains
independent of LLM routing; provider adapters live in the runtime/application
boundary and interpret already-saved outputs.
...
The June 2026 CLI/runtime/editor upgrade pass was checked with:
\begin{verbatim}
python -m pytest -q tests/test_cli_runtime.py \
  tests/test_release_surfaces.py \
  tests/test_cdfl_vscode_extension.py
python cdfd.py doctor --json
python cdfd.py gallery --steps 1 --nx 4 --ny 4 --json
\end{verbatim}
The recorded focused result was 38 passing tests, \texttt{doctor} reported
11 ok checks, and \texttt{gallery} returned an ok finite-audited envelope. This
is not full validation of the CDFD theory; it is evidence that the command
backend, CDFL example, CDFL workbench tooling, Part II diagnostics, domain
traces, packaging surfaces, and VS Code extension metadata execute coherently.
