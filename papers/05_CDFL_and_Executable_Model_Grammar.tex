...
\texttt{cdfd.py cdfl validate}, \texttt{cdfd.py cdfl lint},
\texttt{cdfd.py cdfl format}, \texttt{cdfd.py cdfl ast},
\texttt{cdfd.py cdfl sample}, execution, diagnostics, and JSON export.
\end{abstract}
...
\texttt{RULE}, \texttt{RUN}, \texttt{OBSERVE}, and \texttt{PATIENT}. Hash
comments are ignored by the lexer, which aligns the runtime with the editor
configuration and keeps annotated model files executable.
...
The runtime now exposes a CDFL workbench as strict result envelopes rather than
only as editor-side convenience:
\begin{itemize}
    \item \texttt{cdfl lint}: shared diagnostics for malformed statements,
    block/list balance, rule syntax, and key-value shape;
    \item \texttt{cdfl format}: deterministic indentation over blocks and lists;
    \item \texttt{cdfl ast}: a JSON-safe summary of parser nodes and attributes;
    \item \texttt{cdfl sample}: the canonical heat-flow model for tutorials,
    tests, and editor smoke checks.
\end{itemize}
These commands are intentionally modest. They make CDFL reviewable and
repeatable while preserving the parser and executor as the source of semantic
truth.

\section{Editor Surface}
The repository now includes \texttt{tools/cdfl-vscode}, the Vura Labs
\emph{CDFL Language Support} extension. It provides \texttt{.cdfl} file
association, TextMate highlighting, snippets, local diagnostics, completions,
hover help, document symbols, formatting, and command-palette actions that call
the same \texttt{cdfd.py cdfl} runtime commands. The extension is a research
ergonomics layer, not a second compiler.
...
The final development target remains to rerun more of the selected discovery set
from CDFL rather than from ad hoc Python scripts. The short path is not to
rewrite every experiment by hand. The current grammar already supports simple
parameter and discovery declarations such as:
\begin{verbatim}
SWEEP pulse_strength [1.0, 2.0, 4.0]
DISCOVER {
  target: vacuum_memory
  output: discovery_vacuum_hysteresis
  falsify: residual_vanishes_under_controls
}
\end{verbatim}
The remaining work is to connect richer experiment families, manifests, and HDF5
outputs to these declarations. Once that bridge is complete, selected discoveries
can be run through \texttt{cdfd.py cdfl run}, exported as result envelopes, and
displayed by VOS or CDFD Runtime Studio without changing the engine mathematics.
...
target is to make common sweeps, parameter grids, and discovery runs executable
from CDFL manifests without losing Python-level control for advanced
experiments.
