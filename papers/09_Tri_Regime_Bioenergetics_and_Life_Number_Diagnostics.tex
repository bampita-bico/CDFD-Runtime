...
needed because local $\Psi_s$ can be high in one region and low elsewhere.
...
\texttt{experiments/notebooks/discovery\_ool\_phase.py} sweeps a 50 by 50
parameter grid over energy input and geochemical constraint. The report
\texttt{experiments/reports/NEW\_DISCOVERIES\_REPORT.md} describes candidate
points near $\Lambda=1$, and
\texttt{experiments/outputs/discovery\_ool\_phase\_results.h5} stores the output.
...
\begin{itemize}
    ...
    versus cross-axis intervention toy trajectories.
\end{itemize}
These are model diagnostics and hypothesis generators. They are not clinical
advice or validated therapy rules.
