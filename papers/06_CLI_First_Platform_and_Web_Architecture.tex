...
paper diagnostics, debugging, automation, and saved results. An optional web
studio (\texttt{webapp/}) is a visual layer over the same backend, not a separate
physics system. This paper defines that boundary and explains how CLI outputs
flow into optional visual interfaces.
\end{abstract}
...
\begin{enumerate}
    ...
    \item web app.
\end{enumerate}
The current repository has the engine and CLI as primary surfaces. The initial
web studio under \texttt{webapp/} (CDFD Runtime Studio, Streamlit) is optional:
it is a visual layer over the same kernel, \texttt{runtime/diagnostics.py}, and
domain adapters and does not duplicate physics logic.
...
\begin{itemize}
    ...
    \item validating and executing CDFL models;
    \item exporting JSON result envelopes;
    \item app-key boundary checks (\texttt{cdfd.py auth});
...
causal graphs from run history, and the \texttt{origins\_of\_life} domain explorer.
Future CDFD Studio releases may add model building, coherence monitoring, and
deeper result browsing on top of the same CLI backend.
...
application-facing responses. That layer belongs in CDFD Studio, regulated apps,
...
\end{itemize}
The public Streamlit dashboard under \texttt{webapp/dashboard.py} already
implements an initial subset: regime-banded $\Psi_s$ trajectories, $\Phi$ and
$C$ field heatmaps, Part II aromatic source-mix bars, a Life Number calculator,
Granger-style causal edges from run history, and the \texttt{origins\_of\_life}
domain explorer. The web app presents these as model diagnostics and
falsification targets, not as empirical proof.
